...
\subsection{Scope}
	The scope of this project includes developing a new website hosting a web application.
 	This software should allow for the upload of a explorable 3D model file to be viewed in browser and then also into a VR environment. This project also aims to reduce barriers from uploading and viewing files by implementing the Forge API in order to improve the user experience.
...
\subsection{DEFINITIONS, ACRONYMS, AND ABBREVIATIONS}
	...
	\item{FORGE} A collection of API services provided by Autodesk that provide 3D modeling services and tools.

	\item{Forge Viewer} This is one of the APIs in Forge. It displays 3D models from CAD files and also allows
	for user interaction.
	
	\item{Model Derivative API} This is another API in Forge. It can generate different filetypes that we can utilize in the application.
...
\subsection{PRODUCT FUNCTIONS}
	\begin{enumerate}
	 	\item The user is able to choose a model file locally or upload one from another cloud service.
		...
		\item Once connected to a smartphone the model should be able to be viewed in VR with a the use of a VR headset such as Google Cardboard, if supported. 
		
		\item "Viewable" 3D models can be interacted with by the user in the Forge viewer.		
	\end{enumerate}

\subsection{USER CHARACTERISTICS}
	%perhaps easier if itemized
	When finished this product should be usable by anyone that has access to a model file. If the user is wanting to use the VR 
...
\subsubsection{User Interfaces}
	...
	\begin{itemize}
		...
		\item The user will be able interact and manipulate the model inside the viewer using an interface. This interface can rotate, move, or explode a model.
		\item On a smartphone, the webpage loads a compact version of the main site. 
		\item The viewer on the smartphone is fullscreen when connected, and has similar manipulation methods like the webpage.
	\end{itemize}
...
\begin{enumerate}
	\item This will allow the user to be able to upload any models if they have access to the website.
	...
	%\item The file must be a model specific file and the user must have access to it on their machine. 
