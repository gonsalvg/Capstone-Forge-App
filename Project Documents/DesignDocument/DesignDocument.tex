...
\begin{itemize}
	...
	\item[]\textbf{Model Viewer Navigation} This allows for keyboard and mouse input as a new option for controlling the Forge Viewer. Currently, the viewer can support keyboard controls for movement, which ideally can be enabled for an additional source of movement and manipulation for the user. This will essentially enable the model viewer to explore the 3D space around the model, and will be enabled within an extension for the Viewer, which is a javascript file specifically for the Viewer.
